...
const SUITES = [
  ...
  ["test-punch", "correcting a punch — the clock arithmetic, needs no key"],
  ["test-truckfile", "finding a truck and adding its file up — needs no key"],
  ["test-duals", "duals that do not match — needs no key"],
  ["test-tread", "tread that cannot be right — needs no key"],
  ["test-move", "moving a tire to another wheel — needs no key"],
  ["test-pickunit", "finding a unit by typing its number — needs no key"],
  ["test-retread", "how many times a casing has been capped — needs no key"],
  ["test-axlerole", "which axle a wheel is on — needs no key"],
  ...
  ["test-treadboard", "impossible tread on the Tires screen — needs the app served"],
  ["test-moveboard", "moving and swapping tires in a browser — needs the app served"],
  ["test-pickboard", "typing a unit number on a timecard — needs the app served"],
  ["test-retreadboard", "recording a retread's cap count — needs the app served"],
  ["test-analysisboard", "a brand chart per axle — needs the app served"],
